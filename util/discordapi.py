...
async def send_webhook_thread(thread: discord.Thread, content: str,
                              avatar_url: str = "https://i.imgur.com/rpd75Pr.jpg", username: str = "RPG") -> None:
    # Check if the channel is a text channel or a thread
    if isinstance(thread.parent, discord.TextChannel):
        webhook_list = await thread.parent.webhooks()
        ...
        if not webhook:
            webhook = await thread.parent.create_webhook(name="RPGAI")

        # Split content into chunks of 2000 characters or less
        chunks = [content[i:i + 2000] for i in range(0, len(content), 2000)]

        for chunk in chunks:
            await webhook.send(chunk, username=username, avatar_url=avatar_url, thread=thread)
    else:
        print("The channel must be either a TextChannel or a Thread.")
    return
